...
: OPEN-FILE   ( c-addr u fam -- fid ior )   -ROT C0END SWAP  [ 2 1 ]
   OS" OS_Find"  DUP 0= IF  -1  ELSE 0  THEN ;
: CLOSE-FILE   ( fid -- ior )   0  [ 2 0 ] OS" OS_Find"  0 ;

: CREATE-FILE   ( c-addr u fam -- fid ior )   DROP
   [ HEX ] 80 [ DECIMAL ] OPEN-FILE ;
: RENAME-FILE   ( c-addr1 u1 c-addr2 u2 -- )   C0END -ROT  C0END  25
   [ 3 0 ] OS" OS_FSControl"  0 ;
: DELETE-FILE   ( c-addr u -- ior )   C0END  6  [ 2 1 ] OS" OS_File"  0= ;
...
: FILE-STATUS   ( c-addr u -- x ior )   C0END  5  [ 2 6 ] OS" OS_File"
   NIP NIP NIP NIP ;