...
\ FIXME: Hack for Mit only
"PLATFORM S" Mit" COMPARE 0= [IF]
   : [']   BL WORD FIND 0= IF UNDEFINED THEN  [ ' ADDRESS-LITERAL CURRENT-COMPILE, ]  POSTPONE >NAME POSTPONE ?FIND ;
   IMMEDIATE COMPILING
[THEN]

: POSTPONE   BL WORD FIND ?DUP 0= IF UNDEFINED THEN 0> IF >COMPILE @
   CURRENT-COMPILE, ELSE [ ' ADDRESS-LITERAL CURRENT-COMPILE, ] C" (POSTPONE)" ?FIND CURRENT-COMPILE, THEN ;
IMMEDIATE COMPILING
